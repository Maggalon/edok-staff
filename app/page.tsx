...
export default function Home() {
  ...
  const getBranchItems = async () => {
    if (items && items[0].branchid === selectedBranch!.id) return
                ...
                return (
                  <div key={item.id} onClick={() => setSelectedItem(item)} className={`flex items-center justify-start w-full gap-2 shadow-md border rounded-lg p-2 ${selectedItem === item && "bg-primary-200"}`}>
                    ...
                    <div className="flex-1 flex flex-col items-start gap-1">
                      ...
                      {/* <div className="flex items-center gap-1 font-semibold"><Star className="w-4 h-4 text-primary-600 fill-primary-600" />{selectedBranch?.votesnumber ? selectedBranch!.ratingsum / selectedBranch!.votesnumber : "--"}</div> */}
                    </div>
